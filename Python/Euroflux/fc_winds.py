...
#corr = np.corrcoef(data['Wndir'], data['FC'])
...
#plt.scatter(data['WD'], data['FC'])
ax.plot(data.index, data['FC']['mean'], 'g', linewidth=2.0)
ax.plot(data.index, data['FC']['75%'], color='g', linestyle = 'dashed')
ax.plot(data.index, data['FC']['25%'], color='g', linestyle = 'dashed')
